...
describe("Berry contract", function () {
  it("Show group creation and plan payment", async function () {
    ...
    const groupID = await berry.connect(user1).createGroup(0, 0, 'mi grupo chido', { value: ethers.utils.parseEther('15.0') });
    const groupID2 = await berry.connect(user1).createGroup(1, 0, 'mi grupo chido de udemy', { value: ethers.utils.parseEther('15.0') });
    ...
    group = await berry.groups(0);
    group2 = await berry.groups(1);
    console.log(` dedspues Members in el gurpo 0 : ${group.numMembers.toString()}; Members in el gurpo 1 : ${group2.numMembers.toString()}`)
